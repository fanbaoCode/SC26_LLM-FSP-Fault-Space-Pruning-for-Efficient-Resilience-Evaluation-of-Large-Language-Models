...
batch_size = len(selected_samples)   # 或自定义一个合适的 batch_size
...
# 故障注入试验（批次处理）
print(f"=== {num_trials} 次随机双比特注入试验（批次处理） ===")
...
with open(f"{output_dir}/qwen2-7b_fi_neuron_1bit_prefill_FP32.json", "w") as f:
    json.dump(results, f, indent=4)

print(f"实验完成。结果保存至 '{output_dir}/qwen2-7b_fi_neuron_1bit_prefill_FP32.json'。")