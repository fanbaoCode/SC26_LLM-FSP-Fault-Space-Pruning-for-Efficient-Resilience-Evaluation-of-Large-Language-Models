...
for idx in range(batch_size):
    ...
    print()
    # 获取基准答案prompt长度
    baseline_answer_tokens = tokenizer(baseline_answers[idx], return_tensors="pt", padding=True, truncation=True)
    ...
    try:
        ...
        for length in prompt_lens_baseline_answers:
            if length == 1:
                ...
                target_generate.append(random.randint(2, 30))
        ...
        #     "fault_info": pfi_model.last_faults if hasattr(pfi_model, 'last_faults') else None
        # }
        # logging.error(f"试验 {i + 1} 失败: {error_info}")
        # for idx in range(batch_size):
        #     sample_results[idx]["faulty_outputs"].append({
        #         "trial": i + 1,
        #         "output": None,
        #         "fault_info": None,
        #         "error": error_info
...
# 保存结果
output_dir = "./results/qwen2-7b/squad"
with open(f"{output_dir}/qwen2-7b_fi_neuron_1bit_decode_FP32.json", "w") as f:
    json.dump(results, f, indent=4)

print(f"实验完成。结果保存至 '{output_dir}/qwen2-7b_fi_neuron_1bit_decode_FP32.json'。")